...
/**
 * Note: We no longer need to call Clerk's API directly since we get
 * all necessary user information from the JWT token via ctx.auth.getUserIdentity()
 * This avoids the "Can't use fetch() in queries and mutations" error.
 */
...
export async function requireUserWithAutoSync(ctx: MutationCtx) {
  ...
  if (!user) {
    // Auto-sync user from Clerk JWT token data (no API calls needed)
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) {
      throw new Error("Unable to get user identity from JWT token");
    }

    // Extract user info from JWT token claims with proper type casting
    const email = typeof identity.email === 'string' ? identity.email :
                  typeof identity.emailVerified === 'string' ? identity.emailVerified : undefined;
    const name = typeof identity.name === 'string' ? identity.name :
                 typeof identity.given_name === 'string' ? identity.given_name :
                 typeof identity.nickname === 'string' ? identity.nickname : "Unknown User";
    const image = typeof identity.picture === 'string' ? identity.picture : undefined;
...
/**
 * Test basic auth without user sync
 */
export const testAuth = query({
  ...
  handler: async (ctx) => {
    try {
      const identity = await ctx.auth.getUserIdentity();
      if (!identity) {
        return { success: false, error: "No identity found - using basic ConvexProvider without Clerk integration" };
      }
      return {
        success: true,
        identity: {
...
/**
 * Create user manually with Clerk data (bypasses JWT integration)
 */
export const createUserManually = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingUser) {
      return { success: true, userId: existingUser._id, message: "User already exists" };
    }
