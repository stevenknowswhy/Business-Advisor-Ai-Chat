...
// Get all active advisors with full persona data (for migration)
export const getAllActiveAdvisorsForMigration = query({
  args: {},
  handler: async (ctx) => {
    return await ctx.db
      .query("advisors")
...
// Get advisor by ID with full persona data (for migration)
export const getAdvisorByIdForMigration = query({
  args: { advisorId: v.id("advisors") },
  handler: async (ctx, args) => {
    return await ctx.db.get(args.advisorId);
  },
...
// Seed sample advisors for demo/testing purposes
export const seedSampleAdvisors = mutation({
  args: {},
  handler: async (ctx) => {
    // Check if advisors already exist
    const existing = await ctx.db.query("advisors").collect();
    if (existing.length > 0) {
      return { count: 0, message: "Advisors already exist, skipping seed" };
    }

    const sampleAdvisors = [
      {
        name: "Demo Marketing Advisor",
        title: "Marketing Specialist",
        expertise: "Marketing",
        description: "A sample marketing advisor for testing purposes",
        oneLiner: "Helping you grow your business through strategic marketing"
      },
      {
        name: "Demo Tech Advisor",
        title: "Technology Consultant",
        expertise: "Technology",
        description: "A sample technology advisor for testing purposes",
        oneLiner: "Guiding your technical decisions and architecture"
      },
      {
        name: "Demo Business Advisor",
        title: "Business Strategist",
        expertise: "Business Strategy",
        description: "A sample business advisor for testing purposes",
        oneLiner: "Strategic guidance for business growth and operations"
      }
    ];
    ...
    for (const advisor of sampleAdvisors) {
      await ctx.db.insert("advisors", {
        firstName: advisor.name.split(" ")[1] || "",
        lastName: advisor.name.split(" ")[2] || "",
        imageUrl: undefined,
        ...
        tags: [advisor.expertise.toLowerCase()],
        createdAt: Date.now(),
        updatedAt: Date.now()
      });
      count++;
